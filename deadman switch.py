...
def encrypt_data(file_path, cipher):
    """
...
def encrypt_and_shred_self(file_path, cipher):
    """
    Encrypt, compress, and shred the script file.
...
def shred_file(file_path):
    ...
    Args:
        file_path (str): Path to the file to be shredded.
    """
    # Open the file in write mode and overwrite the content with random data
    with open(file_path, 'wb') as file:
...
def encrypt_compress_and_shred_directory(directory_path, cipher):
    """
